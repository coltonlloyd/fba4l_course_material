...
def _calculate_subset(arguments):
    ...
    reaction_1_name = arguments["reaction1_name"]
    reaction_2_name = arguments["reaction2_name"]
    ...
    reaction1 = model.reactions.get_by_id(reaction_1_name)
    reaction2 = model.reactions.get_by_id(reaction_2_name)
    old_r1_ub = reaction1.upper_bound
    old_r1_lb = reaction1.lower_bound
    old_r2_ub = reaction2.upper_bound
    old_r2_lb = reaction2.lower_bound
    for a, flux1 in enumerate(reaction1_fluxes):
        ...
        reaction1.lower_bound = flux1 - tolerance
        reaction1.upper_bound = flux1 + tolerance

        for b, flux2 in enumerate(reaction2_fluxes):
            j = j_list[b]
            flux2 = float(-1 * flux2)  # same story as flux1
            # change bounds on reaction 2
            reaction2.lower_bound = flux2 - tolerance
            reaction2.upper_bound = flux2 + tolerance
            # solve the problem and save results
            solution = model.slim_optimize(error_value=0)
            if solution:
                results.append((i, j, solution))
            else:
                results.append((i, j, 0, 0, 0))
            # reset reaction 2 bounds
            reaction2.lower_bound = old_r2_lb
            reaction2.upper_bound = old_r2_ub

        # reset reaction 1 bounds
        reaction1.lower_bound = old_r1_lb
        reaction1.upper_bound = old_r1_ub
    return results
...
def calculate_phenotype_phase_plane(
    ...
    metabolite1_name = list(model.reactions.get_by_id(reaction1_name)._metabolites)[0].id
    metabolite2_name = list(model.reactions.get_by_id(reaction2_name)._metabolites)[0].id
    ...
    for n in range(n_processes):
        ...
        arguments_list.append({
            "model": model,
            "reaction1_name": reaction1_name,
            "reaction2_name": reaction2_name,
